 ...
 *
 * -배열 문제점
 * 	: 1)저장할 수 있는 객체 수가 배열을 생성 할 때 결정됨
 * 		불특정 다수의 개체를 저장하기에는 문제가 있다.
 * 	  2)객체 삭제했을 때 해당 인덱스가 비게 됨
 * 		객체를 저장하려면 어디가 비어있는지 확인을 꼭 해야함 . 불편~~
 * -컬렉션 프레임워크
 * 		: 객체들을 효율적으로 추가,삭제,검색 할 수 있도록 제공되는 컬렉션 라이브러리.
 * 		 인터페이스를 통해서 정형화된 방법으로 다양한 컬렉션 클래스를 이용.		
 * 		-컬렉션은 제네릭( Generic ) 기법으로 구현됨.
 * 		-제네릭 : 특정타입만 다루지 않고 여러종류의 타입으로 변신할 수 있도록 
 * 				클래스나 메소드를 일반화시키는 기법
 * 		     	<E><K><V> : 타입 매개 변수  (요소타입을 일반화한타입)
 * 				-예) 제네릭 벡터 : Vector <E>
 * 						E에 특정 타입으로 구체화,
 ...
 *   		 =========== ===========
 * 			= 순서를 유지하고 있다. (각 데이터마다 index 를 가지고 있음)
 * 			= 중복을 허용한다
 * 			= 데이터베이스 프로그램
 * 			
 * 		Set => TreeSet, HashSet
 ...
 * 					======== =======
 * 				아이디 키밸류        패스워드 키밸류
 * 				일반프로그램 =>데이터를 전송할 때 메소드의 매개변수
 * 
 * 
 * 
 * ArrayList => 클래스만 저장이 가능(사용자 정의, 라이브러리 ) 요소로서저장
 * 			add(객체값) 저장(요소 뒤에 붙는다)
...
public class MainClass {
	public static void main(String[] args) {
		...
		list.add(10); //오토박시이잉 실지로 객체가 들어감   Integer   0
